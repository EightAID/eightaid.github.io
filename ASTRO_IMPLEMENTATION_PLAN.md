...
## 第2期リニューアル（2026-07-20）

- トップを『代償少女』中心から「代償プロジェクト」総合ページへ再編
- サークルロゴと横長キービジュアルを使うファーストビューへ変更
- 作品を `/products/[slug]/` の拡張可能な階層へ移行
- 『代償少女』を `/products/daisho-shojo/` へ移し、旧 `/special/` から転送
- 個別メンバープロフィールと過去作品一覧は維持
- プロフィール内の長い説明・現在の取り組みセクションを削除
- 掲載記事・お知らせ用コンテンツコレクションを追加
- 全体を角丸カード・ピル型ボタン中心のUIへ調整
- トップ、作品、メンバーごとに共有カード用OGP画像を設定
- 共有カード画像を横長キービジュアル「メインカプセル」に統一
- 掲載記事を画像なしのプレスリリース・紹介記事一覧へ変更
- 他SNS・外部サイト一覧と活動履歴タイムラインを追加
